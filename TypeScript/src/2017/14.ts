...
for (let i of range(0, 128)) {
  ...
  let bin = [...flatMap<string, string>(x => _.padStart(parseInt(x, 16).toString(2), 4, '0'))(hex)]
  grid.push(bin)
...
let num = 1
strGrid.forEach((r, rIdx) => r.forEach((c, cIdx) => (c === '#' ? mark([rIdx, cIdx], num++) : null)))

function mark([rIdx, cIdx]: number[], _?) {
  if (!strGrid[rIdx] || !strGrid[rIdx][cIdx] || strGrid[rIdx][cIdx] !== '#') return
  strGrid[rIdx][cIdx] = `${num}`
  ;[[rIdx + 1, cIdx], [rIdx, cIdx + 1], [rIdx - 1, cIdx], [rIdx, cIdx - 1]].forEach(mark)
}
console.log(num - 1)
